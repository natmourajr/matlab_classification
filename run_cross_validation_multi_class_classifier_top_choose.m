...
for i_sweep_top = 1:length(s_top)
    for i_cross_validation = 1:CVO.NumTestSets
        fprintf('Top %i (%i Neurons) - Set no %i\n', i_sweep_top, s_top(i_sweep_top), i_cross_validation);
        ...
        cv_SP(i_cross_validation,i_sweep_top) = SP;
...
% checking all SP - per sets
figure;
plot(1:CVO.NumTestSets, mean(cv_SP,2),'bx','LineWidth',2.0);
...
% checking all SP - per top
figure;
plot(min_top:max_top, mean(cv_SP,1),'bx','LineWidth',2.0);
ylabel('% SP','FontSize', 15,'FontWeight', 'bold');
